...
let bTable = makeTable('borrow', 'filteredBorrowings', [
  ...
  {name: 'Barang yang Dipinjam', class: 'px-6 py-3 font-medium'},
  {name: 'Tanggal Pinjam', class: 'px-6 py-3 font-medium'},
  {name: 'Kembali Diharapkan', class: 'px-6 py-3 font-medium'},
  ...
  '<td className="px-6 py-4">{asset ? asset.nama : borrow.items && borrow.items.map ? borrow.items.map((item, idx) => (<div key={idx} className="text-sm"><span className="font-medium text-slate-700">{item.namaBarang}</span> (x{item.jumlah})</div>)) : null}</td>',
...
content = content.replace(/<table className="w-full text-sm text-left">[\s\S]*?<\/table>/, bTable);
