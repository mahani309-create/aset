...
files.forEach(file => {
  ...
  content = content.replace(selectRegex, (match, attributesBefore, classBefore, classAfter, contentInside) => {
    return `<div className="relative w-full sm:w-auto">
              <select${attributesBefore}className="${classBefore}appearance-none bg-white px-4 py-2 pr-10${classAfter} hover:bg-slate-50 transition-colors shadow-sm"${contentInside}</select>
